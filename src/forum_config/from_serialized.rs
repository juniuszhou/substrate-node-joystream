...
pub struct ForumData {
    pub categories: Vec<(CategoryId, Category<BlockNumber, Moment, AccountId>)>,
    pub posts: Vec<(PostId, Post<BlockNumber, Moment, AccountId>)>,
    pub threads: Vec<(ThreadId, Thread<BlockNumber, Moment, AccountId>)>,
}

pub fn parse_forum_json() -> ForumData {
    let data = include_str!("../../res/forum_data_acropolis_serialized.json");
    let forum_data = serde_json::from_str(data);
